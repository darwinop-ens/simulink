...
data.id_buttons = zeros(size(data.consts.ids));
data.selected_read_fields = cell(size(data.consts.ids));
...
for i=1:length(data.consts.ids)
    ...
    if data.consts.ids{i}.address == -1
        ...
        width = 20;
        text = num2str(data.consts.ids{i}.address);
    end
    data.id_buttons(i) = uicontrol(f,'Style','pushbutton', ...
                                     'String',text, ...
                                     'BackgroundColor',data.background_color, ...
                                     'Units','pixels', ...
                                     'Position',[data.consts.ids{i}.posx+8,data.consts.ids{i}.posy-10,width,20], ...
...
if strcmp(get_param(hModel,'lock'),'on') == 0
    ...
    for i = 1:length(data.consts.ids)
        ...
        for j = 1:length(selected_read_fields)
            ...
            if selected_read_fields(j)
                ...
                read_mask_display = [read_mask_display; ...
                                     ...
                                     ',''', ...
                                     data.consts.ids{i}.name, ...
                                     '(', ...
                                     num2str(abs(data.consts.ids{i}.address)), ...
                                     ')/', ...
                                     field.name, ...
                                     ''')' ...
                                    ]; %#ok
        ...
        if data.consts.ids{i}.address >= 0
            instruction = 2;
        else
            instruction = 17;
        end
...
data = guidata(f);
set(f,'Name',sprintf(data.consts.labels{1}, getfullname(data.simulink_block)));
